        ...
        async.forEachSeries(images, function (image, next) {
            ...
            download(image, file, function (err) {
                switch (self.options.server.name) {
                    case 'local':
                        if (self.options.server.baseURI.slice(-1) !== '/') {
                            self.options.server.baseURI += '/';
                        }
                    ...
                    case 'alipay':
                        ...
                        var cdn = new CDN('alipay', self.options.server);
                        ...
                        break;
                    case 'tfsdaily':
                        var CDN = require('../plugin/tfsdaily');
                        var cdn = new CDN('tfsdaily', self.options.server);
                        cdn.upload(file, function (err, url) {
                            data.styleText = data.styleText.replace(new RegExp(image, 'g'), url);
                            next();
                        });
                        break;
                    case 'tfs':
                        var CDN = require('../plugin/tfs');
                        var cdn = new CDN('tfs', self.options.server);
                        cdn.upload(file, function (err, url) {
                            data.styleText = data.styleText.replace(new RegExp(image, 'g'), url);
                            next();
                        });
