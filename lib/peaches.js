...
Peaches.prototype = {
    ...
    local: function () {
        ...
        async.forEachSeries(mediasheet, function (sheet, next) {
            ...
            options.spriteName += new Date().getTime();

            var imageBucket = new ImageBucket(sheet, options);
            imageBucket.downloadImageFile(function (err) {
                if (err) {
                    throw err;
                }
                combine(imageBucket, options, imageBucket.stylesheet, function (err, packers, stylesheet) {
            ...
            var options = _.extend({}, self.options);
            options.spriteName += new Date().getTime();
            var imageBucket = new ImageBucket(stylesheet, options);
            imageBucket.downloadImageFile(function (err) {
                if (err) {
                    throw err;
                }
                combine(imageBucket, options, imageBucket.stylesheet, function (err, packers, sheet) {
                    stylesheet = sheet;
                    if (packers) {
                        packers.forEach(function (packer) {
                            if (packer.server) {
