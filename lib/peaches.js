...
Peaches.prototype = {
    _init: function () {
        ...
        /**
         * 直接调用 API 的程序可能会没有创建响应的目录，
         * 所以这里检查一次，保证目录存在。
         */
        shelljs.mkdir('-p', this.options.server.root);
        shelljs.mkdir('-p', this.options.server.tmp);
    ...
    local: function () {
        ...
        async.forEachSeries(mediasheet, function (sheet, next) {
            ...
            var bucket = new ImageBucket(sheet, options, self);
        ...
        if (this.pom.errors.length) {
            return this.next(this.pom.errors);
        }
