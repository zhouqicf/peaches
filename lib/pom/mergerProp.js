...
Merger.prototype = {
    ...
    merger: function () {
        ...
        this.stylesheet.rules.forEach(function (rule, idx) {
            self.mergerAtRule(rule) || self.mergerRule(_.clone(rule));
        });
        //先插入charset
    ...
    mergerRule: function (rule) {
        ...
        rule.selectors.forEach(function (selector) {
            ...
            rule.declarations.forEach(function (newDecl, idx) {
                var isDuplicate = false, duplicateDecl;
                self.rules[selector].declarations.forEach(function (decl) {
                ...
                if (isDuplicate) {
                    if (newDecl.important) {
                        duplicateDecl.value = newDecl.value;
                    }
                    else if (!duplicateDecl.important) {
                        duplicateDecl.value = newDecl.value;
                    }
                }
                else {
