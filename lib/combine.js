...
3. 使用云端模式：peaches --cloud=http://cloud.peaches.io/api ';
            ...
            canvasImage.onload = function () {
                ...
                // update: 间隙与window.devicePixelRatio相关，retina需要增加2px，2px对内存的消耗不大，统一增加 by zhouqicf@gmail.com
                var block = {
                    w: canvasImage.width + 2,
                    h: canvasImage.height + 2,
                    path: file_path,
    ...
    updatePosition: function (packer, block, x, y, index) {
        ...
        this.stylesheet.rules.forEach(function (rule, idx) {
            ...
            if (width.length === 1) {
                ...
                if (width === 'auto') {
                    if (self.isRetinaSupport) {
                        if (block.canvasImage.width % 2 === 1) {
                            logger.start(block.image.url);
                            logger.warn('§ 警告：为了最大的兼容性，Retina模式下，请避免出现奇数宽高！§');
                            logger.end("宽度：%s", block.canvasImage.width);
                        }
                        rule.setDeclarationValue('width', (block.canvasImage.width / 2) + 'px');
                    }
                    else {
            ...
            if (height.length === 1) {
                ...
                if (height === 'auto') {
                    if (self.isRetinaSupport) {
                        if (block.canvasImage.height % 2 === 1) {
                            logger.start(block.image.url);
                            logger.warn('§ 警告：为了最大的兼容性，Retina模式下，请避免出现奇数宽高！§');
                            logger.end("高度：%s", block.canvasImage.height);
                        }
                        rule.setDeclarationValue('height', (block.canvasImage.height / 2) + 'px');
                    }
                    else {
