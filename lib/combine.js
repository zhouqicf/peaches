    ...
    getPackers: function (next) {
        ...
        async.forEach(images, function (image, callback) {
            ...
            canvasImage.onerror = function (e) {
                logger.error('canvas 画图出现故障：', e);
                logger.warn('请选择以下方式解决：');
                logger.warn('1. 重新处理图片：url = ', image.url);
                logger.warn('2. 使用云端模式：peaches --cloud=http://cloud.peaches.io/api');
                logger.warn('3. 如果使用alipayobjects，请使用原始图片而非压缩后的图片：');
                logger.warn('   在图片名称后，添加“_src”。比如：https://i.alipayobjects.com/e/201304/BUpbE2yzV_src.png')
                process.exit(1);
            }
            ...
            fs.readFile(file_path, function (err, buffer) {
                if (err) {
                    logger.error('文件读取出错，file：', file_path);
                    process.exit(1);
                }
